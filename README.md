...
- [How it works](docs/how-it-works.md): the symlink swap, shared vs. per-project context, and the metarepo layout.
- [Reference](docs/reference.md): every `meta-router.sh` command, configuration, setup env vars, and the file manifest.
- [Source repos and worktrees](docs/worktrees.md): declare each project's repos in `repos.yaml`, then cut per-story git worktrees across them.
